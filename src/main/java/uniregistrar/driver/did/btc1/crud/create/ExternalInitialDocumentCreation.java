...
import uniregistrar.driver.did.btc1.util.JsonLDUtil;
...
public class ExternalInitialDocumentCreation {
    ...
    // https://dcdpr.github.io/did-btc1/#external-initial-document-creation
    public Map.Entry<DID, DIDDocument> externalInitialDocumentCreation(DIDDocument intermediateDocument, Integer version, Network network, /* TODO: extra, not in spec */ Map<String, Object> didDocumentMetadata) throws RegistrationException {
        ...
        // Replace all did:btc1:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx values in the initialDocument with the did.

        try {
            initialDocument = objectMapper.readValue(objectMapper.writeValueAsString(initialDocument).replace("did:btc1:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", did.getDidString()), DIDDocument.class);
